...
Yes. With raise of AI tools, the entry barrier for web development is much lower than before. You can use AI tools to assist you in the development process.

### Why prediction markets?

With the popular raise of platforms like Polymarket and Kalshi, we thought it would be interesting challenge compared to the usual ones like building a to-do list, weather app, or a blog.
...
2. **User Experience**: Is the application user-friendly and visually appealing? (Since this is a subjective criterion, we will not expect you to be a designer, but we will look for a clean and intuitive interface that respect best practices of UI/UX design and it is not hostile to the user).
3. **Code Quality**: You may generate the code using AI tools, but the code it is a liability itself, so ask yourself if you would be able to maintain it in the future.
...
Few observations:
 ...
 One everything is done, please submit the repository link on our platform at example.com/submit. We will review your submission and get back to you with feedback.
...
Hank and John are old friends that goes way back to their college days. While Hank was always more interested in the business side of things, John was the tech-savvy one.

While doom-scrolling on X, Hank saw the raise of prediction markets and thought it would be a great business opportunity. He immediately called John and pitched the idea to him. John was intrigued by the idea and agreed to work on it with Hank.

While working on the project, the rise of LLM tools started to revolutionize the way software is developed. Hunk was excited about those new tools that will make John work faster. But this view was not shared by John.
...
1. Main dashboard where users can see all the active markets and their outcomes. All updated in real-time with filters like order by creation date, bet size, ete. (Since there can be many bets, make sure to implement pagination).
...
- This test will use Bun as the runtime environment for the backend, and sqlite as the database. There is no need for other types of databases or runtime environments. If you application is too slow in your tests, think how you can optimize before doing radical things like adding a new service (e.g. redis) or changing the database.
...
- We recommend that usage of AI tools that have agent capabilities, so that you can delegate some of the tasks to the AI and focus on the overall architecture and design of the application. However, make sure to review the code generated by the AI since you may find some bugs or issues that the AI might have missed.
...
2. Try to analyze similar products like Polymarket and Kalshi to get a better understanding of the features and functionalities that you need to implement. Since this is a simple project, do not overcomplicated things by adding features that are not necessary.
